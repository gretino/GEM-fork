...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer,
    ...
    if getattr(trainer.args, "tune_mm_mlp_adapter", False):
        # Only save Adapter
        keys_to_match = ['mm_projector', 'ecg_projector']
        ...
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
            if current_folder.startswith('checkpoint-'):
                ...
                ecg_projector_folder = os.path.join(parent_folder, "ecg_projector")
                os.makedirs(ecg_projector_folder, exist_ok=True)
                torch.save(weight_to_save, os.path.join(ecg_projector_folder, f'{current_folder}.bin'))

            else:
                torch.save(weight_to_save, os.path.join(output_dir, f'mm_projector.bin'))
                torch.save(weight_to_save, os.path.join(output_dir, f'ecg_projector.bin'))
        return
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        model.config.ecg_projector_lr = training_args.mm_projector_lr
